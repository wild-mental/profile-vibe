...
| 위치 | 경로 |
|------|------|
| 프로젝트 | `.agents/skills/chartjs-revenue-bar-chart/` |
| 개인 | `~/.agents/skills/chartjs-revenue-bar-chart/` |

수동 호출: `/skills` 또는 `$chartjs-revenue-bar-chart`. 스킬 변경이 반영되지 않으면 Codex를 재시작한다. 복붙용 전체 소스는 [reference.md](reference.md).
